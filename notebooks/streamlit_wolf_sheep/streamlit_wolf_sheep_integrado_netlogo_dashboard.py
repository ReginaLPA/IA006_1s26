...
def get_season(tick: int, params: Dict) -> str:
    ...
    idx = (tick // max(1, int(params["season_length"]))) % len(seasons)
    return seasons[idx]
...
def run_simulation(params: Dict) -> Tuple[pd.DataFrame, List[Dict]]:
    ...
    for tick in range(1, int(params["n_steps"]) + 1):
        ...
        status_text.markdown(f"**Simulando...** tick {tick}/{params['n_steps']} · estação: `{get_season(tick, params)}`")
...
def netlogo_world_figure(snapshot: Dict, params: Dict):
    ...
    xgap = 1 if params.get("show_grid_lines", True) and width <= 65 else 0
    ygap = 1 if params.get("show_grid_lines", True) and height <= 65 else 0
    ...
    if HAS_PLOTLY:
        ...
        fig.add_trace(
            go.Heatmap(
                ...
                y=list(range(height)),
                zmin=0,
                zmax=float(params["grass_max"]),
                colorscale=[[0.0, "#082016"], [0.45, "#1f6a35"], [1.0, "#8bd94d"]],
                showscale=True,
                colorbar=dict(title="Grama", thickness=12, len=0.72),
                xgap=xgap,
                ygap=ygap,
                hovertemplate="Patch (%{x}, %{y})<br>Grama: %{z:.2f}<extra></extra>",
                name="Grama",
            )
        )
        ...
        if len(ox) > 0:
            fig.add_trace(
                go.Scatter(
                    ...
                    y=oy,
                    mode="markers",
                    marker=dict(symbol="square", size=marker_size + 2, color="#2a2d34", line=dict(color="#59616a", width=1)),
                    name="Obstáculos",
                    hovertemplate="Obstáculo<br>Patch (%{x}, %{y})<extra></extra>",
                )
            )
        ...
        if sheep_ok:
            fig.add_trace(
                go.Scatter(
                    x=[a[0] for a in sheep_ok],
                    y=[a[1] for a in sheep_ok],
                    mode="markers",
                    marker=dict(symbol="circle", size=marker_size, color="#f7f7ef", line=dict(color="#274b37", width=1)),
                    name="🐑 Ovelhas",
                    hovertemplate="Ovelha<br>Patch (%{x}, %{y})<extra></extra>",
                )
            )
        if sheep_sick:
            fig.add_trace(
                go.Scatter(
                    x=[a[0] for a in sheep_sick],
                    y=[a[1] for a in sheep_sick],
                    mode="markers",
                    marker=dict(symbol="circle", size=marker_size + 1, color="#fff1b3", line=dict(color="#ffb000", width=2)),
                    name="🐑 Ovelhas doentes",
                    hovertemplate="Ovelha doente<br>Patch (%{x}, %{y})<extra></extra>",
                )
            )
        if wolves_ok:
            fig.add_trace(
                go.Scatter(
                    x=[a[0] for a in wolves_ok],
                    y=[a[1] for a in wolves_ok],
                    mode="markers",
                    marker=dict(symbol="triangle-up", size=marker_size + 2, color="#ff4d4d", line=dict(color="#3b1111", width=1)),
                    name="🐺 Lobos",
                    hovertemplate="Lobo<br>Patch (%{x}, %{y})<extra></extra>",
                )
            )
        if wolves_sick:
            fig.add_trace(
                go.Scatter(
                    x=[a[0] for a in wolves_sick],
                    y=[a[1] for a in wolves_sick],
                    mode="markers",
                    marker=dict(symbol="triangle-up", size=marker_size + 3, color="#ff9b54", line=dict(color="#ffdc73", width=2)),
                    name="🐺 Lobos doentes",
                    hovertemplate="Lobo doente<br>Patch (%{x}, %{y})<extra></extra>",
                )
            )

        fig.update_layout(
            title=f"Mundo espacial — tick {snapshot['tick']} · {snapshot['season']}",
            height=660,
            margin=dict(l=10, r=10, t=50, b=10),
            paper_bgcolor="#061817",
            plot_bgcolor="#061817",
    ...
    # Fallback Matplotlib se Plotly não estiver instalado.
    fig, ax = plt.subplots(figsize=(9, 9))
    ax.imshow(grass, origin="lower", vmin=0, vmax=float(params["grass_max"]), cmap="Greens")
    oy, ox = np.where(obstacles)
    ax.scatter(ox, oy, marker="s", s=marker_size * 5, c="black", label="Obstáculos")
    if snapshot["sheep"]:
        ax.scatter([a[0] for a in snapshot["sheep"]], [a[1] for a in snapshot["sheep"]], s=marker_size * 4, c="white", label="Ovelhas")
    if snapshot["wolves"]:
        ax.scatter([a[0] for a in snapshot["wolves"]], [a[1] for a in snapshot["wolves"]], s=marker_size * 6, c="red", marker="^", label="Lobos")
    ax.set_title(f"Mundo espacial — tick {snapshot['tick']} · {snapshot['season']}")
    ax.set_xticks([])
    ax.set_yticks([])
...
def line_chart(df: pd.DataFrame, y_cols: List[str], title: str, y_title: str):
    if HAS_PLOTLY:
        ...
        fig.update_xaxes(gridcolor="rgba(255,255,255,.12)")
        fig.update_yaxes(gridcolor="rgba(255,255,255,.12)")
        st.plotly_chart(fig, use_container_width=True)
    else:
        st.line_chart(df.set_index("tick")[y_cols])
    ...
    if HAS_PLOTLY:
        fig = go.Figure(
            go.Bar(
                x=values["Indicador"],
                y=values["Valor"],
                text=[f"{v:.1f}" for v in values["Valor"]],
                textposition="outside",
            )
        )
        fig.update_layout(
            title="Estado final da simulação",
            height=380,
            margin=dict(l=20, r=20, t=55, b=20),
            paper_bgcolor="#061817",
            plot_bgcolor="#061817",
        ...
        st.bar_chart(values.set_index("Indicador"))
...
with st.sidebar.expander("🎬 Execução e visualização", expanded=True):
    params["n_steps"] = st.slider("Número de ticks", 20, 700, int(params["n_steps"]), 20)
    params["frame_interval"] = st.slider("Salvar quadro a cada N ticks", 1, 25, int(params["frame_interval"]), 1)
    ...
    params["random_seed"] = int(st.number_input("Seed aleatória", 0, 999999, int(params["random_seed"])))
...
if "df_result" not in st.session_state or st.session_state.df_result.empty:
    ...
    with m1:
        metric_card("Ticks", f"{int(last['tick'])}", "tempo simulado")
    with m2:
    ...
    with tab_world:
        ...
        if HAS_PLOTLY:
            st.plotly_chart(netlogo_world_figure(snapshot, last_params), use_container_width=True)
        else:
            st.pyplot(netlogo_world_figure(snapshot, last_params))
        ...
        with st.expander("▶️ Reproduzir animação dentro do Streamlit", expanded=False):
            ...
            if st.button("Reproduzir quadros salvos", use_container_width=True):
                ...
                for f in frames:
                    if HAS_PLOTLY:
                        placeholder.plotly_chart(netlogo_world_figure(f, last_params), use_container_width=True)
                    else:
                        placeholder.pyplot(netlogo_world_figure(f, last_params))
                    time.sleep(speed)
