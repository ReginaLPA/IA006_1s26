...
Execute com:
    streamlit run streamlit_wolf_sheep_integrado_netlogo_dashboard.py
"""
...
params_base = {
    ...
    "show_agent_jitter": True,
    "show_agent_counts": True,
    "show_agent_halo": True,
    "live_preview": True,
...
def netlogo_world_figure(snapshot: Dict, params: Dict):
    ...
    Nesta versão, ovelhas e lobos são desenhados com alto contraste, tamanho maior,
    pequeno deslocamento visual dentro da célula e rótulos de contagem quando há vários
    agentes no mesmo patch. Isso evita que os agentes fiquem escondidos pela grama ou
    sobrepostos uns aos outros.
    """
    grass = snapshot["grass"]
    obstacles = snapshot["obstacles"]
    height, width = grass.shape
    marker_size = int(params.get("agent_marker_size", 15))
    jitter_enabled = bool(params.get("show_agent_jitter", True))
    count_enabled = bool(params.get("show_agent_counts", True))
    halo_enabled = bool(params.get("show_agent_halo", True))
    ...
    fig_w = 10.6
    fig_h = 10.6 * (height / max(width, 1))
    fig_h = min(9.4, max(5.8, fig_h))
    ...
    rng = np.random.default_rng(int(snapshot.get("tick", 0)) + 2026)

    def xy(items, jitter=0.22):
        if not items:
            return [], []
        xs = np.array([a[0] for a in items], dtype=float)
        ys = np.array([a[1] for a in items], dtype=float)
        if jitter_enabled and len(items) > 1:
            xs = xs + rng.uniform(-jitter, jitter, size=len(items))
            ys = ys + rng.uniform(-jitter, jitter, size=len(items))
        return xs, ys

    def draw_agent_layer(items, *, marker, face, edge, label, z, scale=1.0, halo_color=None):
        if not items:
            return
        xs, ys = xy(items)
        size = (marker_size * scale) ** 2
        if halo_enabled:
            ax.scatter(
                xs,
                ys,
                s=size * 2.00,
                c=halo_color or face,
                alpha=0.22,
                marker=marker,
                linewidths=0,
                zorder=z - 0.3,
    ...
    # Ovelhas: círculos claros. Lobos: triângulos vermelhos grandes.
    draw_agent_layer(sheep_ok, marker="o", face="#fffdf0", edge="#06301f", label="Ovelhas", z=6, scale=1.20, halo_color="#ffffff")
    draw_agent_layer(sheep_sick, marker="o", face="#ffd84d", edge="#6d3600", label="Ovelhas doentes", z=7, scale=1.32, halo_color="#ffd84d")
    draw_agent_layer(wolves_ok, marker="^", face="#ff3030", edge="#fff2d0", label="Lobos", z=8, scale=1.55, halo_color="#ff3030")
    draw_agent_layer(wolves_sick, marker="^", face="#ff9b2f", edge="#fff2d0", label="Lobos doentes", z=9, scale=1.70, halo_color="#ff9b2f")

    def draw_counts(items, color, dx=0.18, dy=0.18):
        if not (count_enabled and items and width <= 80 and height <= 80):
            return
        counts = {}
        for x, y, _e in items:
        ...
        for (x, y), n in counts.items():
            if n >= 2:
                ax.text(
                    ...
                    str(n),
                    color=color,
                    fontsize=max(7, min(11, marker_size - 2)),
                    fontweight="bold",
                    ha="center",
                    va="center",
                    zorder=10,
                    bbox=dict(boxstyle="round,pad=0.12", facecolor="#061817", edgecolor=color, alpha=0.72, linewidth=0.5),
                )

    draw_counts(sheep_ok + sheep_sick, "#ffffff", dx=0.20, dy=0.22)
    draw_counts(wolves_ok + wolves_sick, "#ff6b6b", dx=-0.22, dy=-0.20)

    if params.get("show_grid_lines", True) and width <= 70 and height <= 70:
        ax.set_xticks(np.arange(-0.5, width, 1), minor=True)
        ax.set_yticks(np.arange(-0.5, height, 1), minor=True)
        ax.grid(which="minor", color="white", alpha=0.10, linewidth=0.35)
    ...
    title = f"Mundo espacial estilo NetLogo — passo {snapshot['tick']} · {snapshot['season']}"
    ax.set_title(title, color="#f3fff8", fontsize=14, fontweight="bold", pad=10)

    # Contadores dentro do próprio mapa, para deixar claro que os agentes existem mesmo se estiverem sobrepostos.
    ax.text(
        0.01,
        0.985,
        f"🐑 Ovelhas: {len(sheep)}    🐺 Lobos: {len(wolves)}",
        ...
        fontsize=11,
        fontweight="bold",
        bbox=dict(boxstyle="round,pad=0.35", facecolor="#061817", edgecolor="#2d6f65", alpha=0.82),
        zorder=20,
    )
...
with st.sidebar.expander("🎬 Execução e visualização", expanded=True):
    ...
    params["agent_marker_size"] = st.slider("Tamanho de ovelhas e lobos", 8, 32, int(params.get("agent_marker_size", 15)), 1)
    params["show_grid_lines"] = st.checkbox("Mostrar grade dos patches", bool(params["show_grid_lines"]))
    params["show_agent_jitter"] = st.checkbox("Separar visualmente agentes sobrepostos", bool(params.get("show_agent_jitter", True)))
    params["show_agent_counts"] = st.checkbox("Mostrar número quando houver vários agentes na mesma célula", bool(params.get("show_agent_counts", True)))
    params["show_agent_halo"] = st.checkbox("Destacar ovelhas e lobos com brilho", bool(params.get("show_agent_halo", True)))
...
if "df_result" not in st.session_state or st.session_state.df_result.empty:
    ...
    with tab_world:
        ...
        st.markdown(
            '<p class="section-note">Verde = grama; quadrados escuros = obstáculos; círculos brancos grandes = ovelhas; triângulos vermelhos grandes = lobos.</p>',
            unsafe_allow_html=True,
        )

        frame_ticks = [f["tick"] for f in frames]
        default_frame_idx = len(frames) - 1
        for i in range(len(frames) - 1, -1, -1):
            if len(frames[i].get("sheep", [])) > 0 and len(frames[i].get("wolves", [])) > 0:
                default_frame_idx = i
                break
        ...
        with col_a:
            frame_idx = st.slider(
                ...
                format="quadro %d",
                help="Use este controle como o slider temporal do NetLogo. Por padrão, o app abre no último quadro em que ainda há ovelhas e lobos visíveis.",
            )
        snapshot = frames[frame_idx]
        with col_b:
            st.metric("Passo", snapshot["tick"])
        with col_c:
            st.metric("Ovelhas", len(snapshot["sheep"]))
        with col_d:
            st.metric("Lobos", len(snapshot["wolves"]))

        if len(snapshot.get("wolves", [])) == 0:
            st.warning("Neste quadro não há lobos para mostrar. Mova o controle 'Passo visualizado' para um quadro anterior ou aumente energia/reprodução dos lobos.")
        if len(snapshot.get("sheep", [])) == 0:
            st.warning("Neste quadro não há ovelhas para mostrar. Mova o controle 'Passo visualizado' para um quadro anterior ou aumente energia/reprodução das ovelhas.")
